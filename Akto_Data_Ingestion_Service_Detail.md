...
**输入格式（IngestDataBatch — 扁平 JSON，所有字段为 String）：**
```json
{
  "batchData": [
    {
...
**处理流程**：
1. 接收批量数据（`List<IngestDataBatch>`，最多 100 条）
2. 调用 `setObserveMode()` 在 tag JSON 中注入 `akto_guardrail_mode = "OBSERVE"`
3. 调用 `KafkaUtils.insertData()` → `buildMessageObject()` 将 IngestDataBatch POJO 转为 `BasicDBObject`
4. `obj.toString()` 序列化为 JSON 字符串，作为 Kafka 消息 value 发布

**输出格式（Kafka 消息 — JSON 字符串）：**
```json
{
  "path": "/api/users/123",
  "requestHeaders": "{\"host\":\"api.example.com\",...}",
...
Data Ingestion Service 做的是**序列化和字段适配**，而非深度类型转换。真正的类型转换发生在下游 api-runtime 的 `parseKafkaMessage()` 中：

| 步骤 | 在哪里 | 做什么 |
|---|---|---|
| 1. POJO → BasicDBObject | Data Ingestion Service (`buildMessageObject`) | 从 `IngestDataBatch` 提取 22 个 String 字段放入 BSON 对象 |
| 2. tag 增强 | Data Ingestion Service (`setObserveMode`) | 在 tag JSON 中注入 `akto_guardrail_mode: "OBSERVE"` |
| 3. 序列化 | Data Ingestion Service (`obj.toString()`) | BasicDBObject → JSON 字符串，作为 Kafka 消息 value |
| 4. 反序列化 | api-runtime (`parseKafkaMessage`) | JSON 字符串 → `HttpResponseParams` 对象 |
| 5. 类型转换 | api-runtime (`parseKafkaMessage`) | String → int (statusCode/time) / boolean (isPending) / Map (headers) / enum (source) |
| 6. 字段映射 | api-runtime (`parseKafkaMessage`) | `path` → `requestParams.url`, `akto_vxlan_id` → `apiCollectionId` |

**输入与输出的字段名差异（Data Ingestion Service 不改名，但下游会）：**

| IngestDataBatch 字段 | Kafka 消息字段 | api-runtime 读取为 | 类型变化 |
|---|---|---|---|
| `path` | `path` | `requestParams.url` | String → String |
| `statusCode` | `statusCode` | `statusCode` | **String → int** |
| `time` | `time` | `time` | **String → int** |
| `requestHeaders` (JSON 字符串) | `requestHeaders` | `requestHeaders` (Map) | **String → Map<String,List<String>>** |
| `responseHeaders` (JSON 字符串) | `responseHeaders` | `responseHeaders` (Map) | **String → Map<String,List<String>>** |
| `akto_vxlan_id` | `akto_vxlan_id` | `apiCollectionId` | **String → int** |
| `is_pending` | `is_pending` | `isPending` | **String → boolean** |
| `source` | `source` | `source` | **String → enum Source** |

> **总结**：Data Ingestion Service 的"转换格式"主要是三件事：(1) POJO → JSON 序列化 (2) tag 增强（注入 guardrail_mode）(3) 确保字段名与下游解析逻辑一致。真正的类型转换（String → int/boolean/Map/enum）由 api-runtime 完成。
